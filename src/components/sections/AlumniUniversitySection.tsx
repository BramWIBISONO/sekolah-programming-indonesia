  ...
  return (
    <section className="py-20 bg-slate-50 border-y border-slate-100 overflow-hidden relative">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12 relative z-10">
        ...
        <div className="md:hidden relative flex overflow-x-hidden">
          ...
          >
            {[...UNIVERSITIES_DATA, ...UNIVERSITIES_DATA].map((uni, idx) => (
              <div key={`${uni.shortName}-${idx}`} className="w-28 h-20 bg-white rounded-2xl border border-slate-200/80 shadow-sm flex items-center justify-center p-4 shrink-0 grayscale hover:grayscale-0 transition-all duration-300">
                <ImageWithFallback
        ...
        {/* Desktop: Spaced Grid */}
        <div className="hidden md:grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 lg:gap-8">
          {UNIVERSITIES_DATA.map((uni) => (
            <div key={uni.shortName} className="group bg-white rounded-3xl border border-slate-200 shadow-sm hover:shadow-xl hover:border-blue-200 hover:-translate-y-1 transition-all duration-300 h-32 flex items-center justify-center p-6">
              <div className="w-full h-full relative flex items-center justify-center grayscale opacity-70 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-500">
                <ImageWithFallback
                  src={getLogoPath(uni.logoAssetKey)}
                  alt={uni.name}
                  fallbackLabel={uni.shortName}
